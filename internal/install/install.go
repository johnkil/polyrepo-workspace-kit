...
func portableTargets(root string, opts PlanOptions) ([]Target, []string, error) {
	...
	case ScopeUser:
		...
		targets, err := plannedSkillTargets(root, filepath.Join(absUserRoot, ".agents", "skills"), ScopeUser, "portable", "portable", absUserRoot)
		if err != nil {
			return nil, nil, err
		}
...
func portableRepoTargets(root string, opts PlanOptions, tool string) ([]Target, error) {
	...
	skillTargets, err := plannedSkillTargets(root, filepath.Join(checkout, ".agents", "skills"), ScopeRepo, tool, "portable", checkout)
	if err != nil {
		return nil, err
	}
...
func claudeTargets(root string, opts PlanOptions) ([]Target, []string, error) {
	...
	skillTargets, err := plannedSkillTargets(root, filepath.Join(checkout, ".claude", "skills"), ScopeRepo, "claude", "claude", checkout)
	if err != nil {
		return nil, nil, err
	}
	targets = append(targets, skillTargets...)
...
%s
`, repoID, repoID, workspaceDoc.Workspace.Model, renderReadFirst(repoDoc.ReadFirst), renderEntrypoints(repoDoc.Entrypoints), rules), nil
}
...
%s
`, repoID, repoID, repoID, renderEntrypoints(repoDoc.Entrypoints), rules), nil
}
...
%s
`, repoID, repoID, rules), nil
}

func plannedSkillTargets(root string, dstRoot string, scope Scope, tool string, source string, boundaryRoot string) ([]Target, error) {
	sourceRoot := filepath.Join(root, "guidance", "skills")
	if err := pathWithinBoundary(root, sourceRoot); err != nil {
		return nil, fmt.Errorf("guidance skills root is unsafe: %w", err)
	}
...
func renderedRules(root string) (string, error) {
	rulesRoot := filepath.Join(root, "guidance", "rules")
	if err := pathWithinBoundary(root, rulesRoot); err != nil {
		return "", fmt.Errorf("guidance rules root is unsafe: %w", err)
	}
	entries, err := os.ReadDir(rulesRoot)
	if err != nil {
		return "_No shared guidance rules defined._", nil
	}
	var blocks []string
	for _, entry := range entries {
		...
		data, err := readFileWithinBoundary(rulesRoot, path)
		if err != nil {
			return "", fmt.Errorf("guidance rule %s is unsafe: %w", entry.Name(), err)
		}
...
func evaluateTarget(target Target, opts PlanOptions) Target {
	if err := targetPathError(target); err != nil {
		return blockedUnsafeTarget(target, "unsafe target path: "+err.Error())
	}
	if err := sourcePathError(target); err != nil {
		return blockedUnsafeTarget(target, "unsafe source path: "+err.Error())
	}
...
func blockedUnsafeTarget(target Target, note string) Target {
...
func applyTarget(target Target) error {
	if err := targetPathError(target); err != nil {
		return fmt.Errorf("unsafe install target %s: %w", target.Path, err)
	}
	if err := sourcePathError(target); err != nil {
		return fmt.Errorf("unsafe install source %s: %w", target.SourcePath, err)
	}
	if target.Status == StatusBackupOverwrite {
...
func DiffTarget(target Target) []string {
	...
	if err := sourcePathError(target); err != nil {
		return []string{fmt.Sprintf("# diff unavailable for %s: unsafe source path: %v\n", target.Path, err)}
	}
...
func desiredText(target Target) (string, bool) {
	...
	if target.SourcePath != "" {
		...
		if target.SourceRoot != "" {
			data, err = readFileWithinBoundary(target.SourceRoot, target.SourcePath)
		} else {
...
func sourcePathError(target Target) error {
	if target.SourcePath == "" || target.SourceRoot == "" {
		return nil
	}
	if err := pathWithinBoundary(target.SourceRoot, target.SourcePath); err != nil {
		return err
	}
	info, err := os.Lstat(target.SourcePath)
	if err != nil {
		return err
	}
	if info.IsDir() {
		return fmt.Errorf("%s is a directory", target.SourcePath)
	}
	return nil
}

func readFileWithinBoundary(root string, path string) ([]byte, error) {
	if err := pathWithinBoundary(root, path); err != nil {
		return nil, err
	}
